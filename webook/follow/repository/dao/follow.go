...
func (f *FollowRelationGormDAO) FollowRelationList(ctx context.Context, follower int64, limit, offset int) ([]FollowRelation, error) {
	...
	err := f.db.WithContext(ctx).Where("follower = ? AND status = ?", follower, FollowRelationStatusActive).Offset(offset).Limit(limit).Find(&res).Error
	return res, err
}

func (f *FollowRelationGormDAO) FansList(ctx context.Context, followee int64, limit, offset int) ([]FollowRelation, error) {
	var res []FollowRelation
	err := f.db.WithContext(ctx).Where("followee = ? AND status = ?", followee, FollowRelationStatusActive).Offset(offset).Limit(limit).Find(&res).Error
	return res, err
}

func (f *FollowRelationGormDAO) FollowRelationDetail(ctx context.Context, follower, followee int64) (FollowRelation, error) {
	var res FollowRelation
	err := f.db.WithContext(ctx).Where("follower = ? AND followee = ? AND status = ?", follower, followee, FollowRelationStatusActive).First(&res).Error
	return res, err
}
