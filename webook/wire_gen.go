...
func InitWebServer() *App {
	...
	rewardServiceClient := ioc.InitRewardClient(clientv3Client)
	articleHandler := web.NewArticleHandler(logger, articleService, interactiveServiceClient, rewardServiceClient)
	wechatService := ioc.InitWechatService(logger)
	oAuth2WechatHandler := web.NewOAuth2WechatHandler(wechatService, userService, handler)
	rewardHandler := web.NewRewardHandler(rewardServiceClient)
	engine := ioc.InitWebServer(v, userHandler, articleHandler, oAuth2WechatHandler, rewardHandler)
