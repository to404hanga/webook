...
func (s *ArticleHandlerSuite) SetupSuite() {
	s.db = startup.InitDB()
	hdl := startup.InitArticleHandler(articleDao.NewGormArticleDAO(s.db))
	server := gin.Default()
	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				var publishedArt articleDao.PublishedArticle
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				s.db.Create(&art)
				part := articleDao.PublishedArticle(art)
				s.db.Create(&part)
			},
			after: func(t *testing.T) {
				var art articleDao.Article
				...
				var part articleDao.PublishedArticle
				...
				// 创建时间没变
				assert.Equal(t, int64(456), part.CreateTime)
				// 更新时间变了
				assert.True(t, part.UpdateTime > 234)
			},
			req: Article{
				Id:      3,
				Title:   "新的标题",
		...
		},
		{
			...
			before: func(t *testing.T) {
				...
				s.db.Create(&art)
				part := articleDao.PublishedArticle(articleDao.Article{
					Id:         4,
					Title:      "我的标题",
					Content:    "我的内容",
					CreateTime: 456,
			...
			after: func(t *testing.T) {
				...
				assert.Equal(t, int64(456), art.CreateTime)
				assert.Equal(t, int64(234), art.UpdateTime)
				...
				assert.Equal(t, int64(456), part.CreateTime)
				// 更新时间变了
				assert.Equal(t, int64(234), part.UpdateTime)
			},
			req: Article{
				Id:      4,
				Title:   "新的标题",
