...
func TestFailoverSMSService_Send(t *testing.T) {
	testCases := []struct {
		name    string
		mocks   func(ctrl *gomock.Controller) []service.Service
		wantErr error
	}{
		// {
		// 	name: "一次发送成功",
		...
		// },
		{
			...
			mocks: func(ctrl *gomock.Controller) []service.Service {
				...
				svc0.EXPECT().Send(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(errors.New("发送失败")).AnyTimes()
		...
		// 	},
		// 	wantErr: errors.New("轮询了所有服务商，但是都发送失败"),
		// },
	}
