...
func NewOAuth2WechatHandler(svc oauth2v1.Oauth2ServiceClient, userSvc userv1.UserServiceClient, handler myJwt.Handler) *OAuth2WechatHandler {
...
func (h *OAuth2WechatHandler) Auth2URL(ctx *gin.Context) {
	state := uuid.New()
	resp, err := h.svc.AuthURL(ctx.Request.Context(), &oauth2v1.AuthURLRequest{
		State: state,
	})
	if err != nil {
...
func (h *OAuth2WechatHandler) Callback(ctx *gin.Context) {
	...
	code := ctx.Query("code")
	wechatInfo, err := h.svc.VerifyCode(ctx.Request.Context(), &oauth2v1.VerifyCodeRequest{
		Code: code,
	})
	if err != nil {
		ctx.JSON(http.StatusOK, ginx.Result{
	...
	user, err := h.userSvc.FindOrCreateByWechat(ctx.Request.Context(), &userv1.FindOrCreateByWechatRequest{
		Info: &userv1.WechatInfo{
			OpenId:  wechatInfo.GetOpenId(),
			UnionId: wechatInfo.GetUnionId(),
		},
	})
	if err != nil {
		ctx.JSON(http.StatusOK, ginx.Result{
